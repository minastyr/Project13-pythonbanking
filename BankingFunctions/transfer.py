...
def handle_transfer(checking, savings):
    ...
    try:
        ...
        if account_choice in ['1', '2']:
            try:
                # Prompt the user to enter the amount to transfer and convert it to a float.
                amount = float(input("Enter the amount to transfer: $"))
            except ValueError:
                # Print an error message if the user enters an invalid amount.
                print("Please enter a dollar amount.")
                # Call the handle_transfer function recursively if the user enters an invalid amount.
            ...
            if account_choice == '1':
                try:
                    # Call the withdraw and deposit methods on the appropriate account.
                    checking.withdraw(amount)
                    savings.deposit(amount)
                    print(f"Transfer successful: ${amount:,.2f} from Checking to Savings.")
                except ValueError as e:
                    print(e)
                    return handle_transfer(checking, savings)
            else:
                try:
                    # Call the withdraw and deposit methods on the appropriate account.
                    savings.withdraw(amount)
                    checking.deposit(amount)
                    print(f"Transfer successful: ${amount:,.2f} from Savings to Checking.")
                except ValueError as e:
                    print(e)
                    return handle_transfer(checking, savings)
